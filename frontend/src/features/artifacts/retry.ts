/**
 * Artifacts retry helper — exponential backoff with jitter per CLAUDE.md §9.
 *
 * Purpose:
 *   Implement the retry policy from CLAUDE.md §9 for idempotent artifacts API
 ...
 *   - Classify errors via isTransientArtifactError() before retrying.
 ...
 *   - Know about axios, artifact domain, or React.
 */

import { isTransientArtifactError } from "./errors";
import {
...
export interface RetryOptions {
  /** Maximum retry attempts after the initial call. Default: ARTIFACTS_API_MAX_RETRIES. */
  maxRetries?: number;
...
export async function retryWithBackoff<T>(
  ...
  const {
    maxRetries = ARTIFACTS_API_MAX_RETRIES,
    baseDelayMs = ARTIFACTS_API_RETRY_BASE_DELAY_MS,
    jitterFactor = ARTIFACTS_API_JITTER_FACTOR,
    signal,
    onRetry,
    isTransient = isTransientArtifactError,
