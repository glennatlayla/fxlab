/**
 * Queues retry helper — exponential backoff with jitter per CLAUDE.md §9.
 *
 * Purpose:
 *   Implement the retry policy from CLAUDE.md §9 for idempotent queues API
 ...
 *   - Classify errors via isTransientQueuesError() before retrying.
 ...
 *   - Know about axios, queue domain, or React.
 */

import { isTransientQueuesError } from "./errors";
import {
...
export interface RetryOptions {
  /** Maximum retry attempts after the initial call. Default: QUEUES_API_MAX_RETRIES. */
  maxRetries?: number;
...
export async function retryWithBackoff<T>(
  ...
  const {
    maxRetries = QUEUES_API_MAX_RETRIES,
    baseDelayMs = QUEUES_API_RETRY_BASE_DELAY_MS,
    jitterFactor = QUEUES_API_JITTER_FACTOR,
    signal,
    onRetry,
    isTransient = isTransientQueuesError,
