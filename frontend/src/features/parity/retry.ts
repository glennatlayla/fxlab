/**
 * Parity retry helper — exponential backoff with jitter per CLAUDE.md §9.
 *
 * Purpose:
 *   Implement the retry policy from CLAUDE.md §9 for idempotent parity API
 ...
 *   - Classify errors via isTransientParityError() before retrying.
 ...
 *   - Know about axios, parity domain, or React.
 */

import { isTransientParityError } from "./errors";
import {
...
export interface RetryOptions {
  /** Maximum retry attempts after the initial call. Default: PARITY_API_MAX_RETRIES. */
  maxRetries?: number;
...
export async function retryWithBackoff<T>(
  ...
  const {
    maxRetries = PARITY_API_MAX_RETRIES,
    baseDelayMs = PARITY_API_RETRY_BASE_DELAY_MS,
    jitterFactor = PARITY_API_JITTER_FACTOR,
    signal,
    onRetry,
    isTransient = isTransientParityError,
