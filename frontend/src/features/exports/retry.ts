/**
 * Exports retry helper — exponential backoff with jitter per CLAUDE.md §9.
 *
 * Purpose:
 *   Implement the retry policy from CLAUDE.md §9 for idempotent exports API
 ...
 *   - Classify errors via isTransientExportError() before retrying.
 ...
 *   - Know about axios, export domain, or React.
 */

import { isTransientExportError } from "./errors";
import {
...
export interface RetryOptions {
  /** Maximum retry attempts after the initial call. Default: EXPORTS_API_MAX_RETRIES. */
  maxRetries?: number;
...
export async function retryWithBackoff<T>(
  ...
  const {
    maxRetries = EXPORTS_API_MAX_RETRIES,
    baseDelayMs = EXPORTS_API_RETRY_BASE_DELAY_MS,
    jitterFactor = EXPORTS_API_JITTER_FACTOR,
    signal,
    onRetry,
    isTransient = isTransientExportError,
