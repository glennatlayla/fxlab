/**
 * Audit retry helper — exponential backoff with jitter per CLAUDE.md §9.
 *
 * Purpose:
 *   Implement the retry policy from CLAUDE.md §9 for idempotent audit API
 ...
 *   - Classify errors via isTransientAuditError() before retrying.
 ...
 *   - Know about axios, audit domain, or React.
 */

import { isTransientAuditError } from "./errors";
import {
...
export interface RetryOptions {
  /** Maximum retry attempts after the initial call. Default: AUDIT_API_MAX_RETRIES. */
  maxRetries?: number;
...
export async function retryWithBackoff<T>(
  ...
  const {
    maxRetries = AUDIT_API_MAX_RETRIES,
    baseDelayMs = AUDIT_API_RETRY_BASE_DELAY_MS,
    jitterFactor = AUDIT_API_JITTER_FACTOR,
    signal,
    onRetry,
    isTransient = isTransientAuditError,
