...
def test_results_endpoint_requires_valid_ulid_format(client):
    ...
    response = client.get(f"/runs/{invalid_run_id}/results")
...
def test_results_endpoint_returns_404_for_nonexistent_run(client):
    ...
    with patch("services.api.main.get_run_results") as mock_get:
        ...
        response = client.get(f"/runs/{nonexistent_run_id}/results")
...
def test_results_endpoint_handles_service_errors_gracefully(client):
    ...
    with patch("services.api.main.get_run_results") as mock_get:
        ...
        assert response.status_code == 500, \
            "Service errors must return 500 Internal Server Error"
...
def test_results_endpoint_does_not_compute_readiness_locally(client):
    ...
    with patch("services.api.main.get_run_results") as mock_get:
        ...
        assert "readiness_grade" not in data, \
            "Results endpoint must not compute readiness grades"
        assert "governance_state" not in data, \
            "Results endpoint must not compute governance state"
