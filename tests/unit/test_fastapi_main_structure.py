...
def test_ac1_fastapi_app_is_fastapi_instance():
    ...
    Uses type name check instead of isinstance() because OpenTelemetry
    instrumentation can replace FastAPI with _InstrumentedFastAPI at
    import time, causing isinstance(app, FastAPI) to fail depending
    on test ordering (the observability module instruments the class).
    """
    from services.api.main import app

    # Walk the MRO to check for FastAPI in the inheritance chain.
    # This handles both vanilla FastAPI and OTel-instrumented subclasses.
    mro_names = [cls.__name__ for cls in type(app).__mro__]
    assert "FastAPI" in mro_names, f"app must be a FastAPI instance (MRO: {mro_names})"
...
def test_ac2_health_endpoint_does_not_require_auth():
    ...
    assert response.status_code not in [401, 403], "Health endpoint must not require authentication"
