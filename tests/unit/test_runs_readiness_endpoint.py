...
def test_readiness_endpoint_requires_valid_ulid(client):
    ...
    response = client.get(f"/runs/{invalid_run_id}/readiness", headers=AUTH_HEADERS)

    assert response.status_code in [400, 422], "Invalid ULID must be rejected"
...
def test_readiness_endpoint_returns_404_for_nonexistent_run(client):
    ...
    with patch("services.api.main.get_readiness_report") as mock_get:
        ...
        response = client.get(f"/runs/{nonexistent_run_id}/readiness", headers=AUTH_HEADERS)
...
def test_readiness_response_includes_grade_and_blockers(client):
    ...
    mock_report = {"run_id": test_run_id, "readiness_grade": "GREEN", "blockers": []}
    ...
    with patch("services.api.main.get_readiness_report") as mock_get:
        ...
        assert "readiness_grade" in data, "Response must include readiness_grade"
        assert "blockers" in data, "Response must include blockers list"
...
def test_readiness_blockers_include_owner_and_next_step(client):
    ...
    with patch("services.api.main.get_readiness_report") as mock_get:
        ...
        assert "blocker_owner" in first_blocker, "Blocker must include blocker_owner field"
...
def test_readiness_endpoint_is_read_only(client):
    ...
    response = client.post(f"/runs/{test_run_id}/readiness", headers=AUTH_HEADERS)
    assert response.status_code == 405, "Readiness endpoint must not accept POST"

    # Try PUT
    response = client.put(f"/runs/{test_run_id}/readiness", headers=AUTH_HEADERS)
    assert response.status_code == 405, "Readiness endpoint must not accept PUT"
...
def test_readiness_report_includes_scoring_evidence(client):
    ...
    with patch("services.api.main.get_readiness_report") as mock_get:
        ...
        assert "scoring_evidence" in data, "Readiness report must include scoring evidence"
