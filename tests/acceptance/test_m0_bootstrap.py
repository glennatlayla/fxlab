...
def test_service_directories_exist():
    ...
    for svc in services:
        assert (PROJECT_ROOT / "services" / svc).is_dir(), f"services/{svc}/ directory missing"
...
def test_lib_directories_exist():
    ...
    for lib in libs:
        assert (PROJECT_ROOT / "libs" / lib).is_dir(), f"libs/{lib}/ directory missing"


def test_infra_directories_exist():
    """Infra directories exist."""
    for d in ["compose", "docker", "migrations", "observability"]:
        assert (PROJECT_ROOT / "infra" / d).is_dir(), f"infra/{d}/ directory missing"
...
def test_tracking_files_exist():
    ...
    for ext in (".progress", ".issues", ".lessons-learned"):
        ...
        content = p.read_text()
        assert f"# Workplan: {stem}.md" in content, f"Header 'Workplan:' missing in {p.name}"

    assert (tracking / "SHARED_LESSONS.md").exists(), "SHARED_LESSONS.md missing"
    assert (tracking / ".active_workplan").exists(), ".active_workplan missing"
...
def test_fixture_csv_headers():
    ...
    assert expected_headers <= actual, f"Missing headers: {expected_headers - actual}"
...
def test_malformed_fixture_has_expected_anomalies():
    ...
    has_negative_vol = any(float(r["volume"]) < 0 for r in rows if r["volume"])
...
def test_parity_mismatch_differs_from_left():
    ...
    diffs = sum(
        1 for left_row, m in zip(left, mismatch, strict=False) if left_row["close"] != m["close"]
    )
...
def test_contracts_importable():
    ...
    from libs.contracts import base, enums, errors  # noqa: F401
